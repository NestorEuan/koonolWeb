...
class CorteCajaMdl extends Model
{
    ...
    public function getRegistros($id = false, $idsuc = false, $paginado = false, $activos = false)
    {
        ...
        // $m = $this->builder->where(['nIdCorte' => $id])->getCompiledSelect(false);
    ...
    public function getLstCorteCajero($idCorte)
    {
        ...
        return $this->db->query(
            'SELECT a.nIdVentas, IFNULL(b.nFolioFactura, 0) AS nFactura, ' .
                ...
                ') AS cadPagos, SUM(c.nDescuentoTotal) AS sumaDescuento ' .
                'FROM vtventas a ' .
    ...
    public function getLstCorteEScaja($id)
    {
        return $this->db->query(
    ...
    public function getLstCorteDepositos($id)
    {
        return $this->db->query(
    ...
    // $soloUsuarios indica que se devuelve un listado de usuarios que entran en el corte
