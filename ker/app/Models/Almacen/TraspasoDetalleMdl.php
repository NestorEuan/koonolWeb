...
class TraspasoDetalleMdl extends Model{
    protected $table = 'altraspasodetalle';

    protected $allowedFields = ['nIdTraspaso', 'nIdArticulo', 'fCantidad', 'fImporte', 'fRecibido', 'fPorRecibir' ];

    protected $primaryKey = 'nIdTraspasoDetalle';
    ...
    public function updtRecibidos( $id, $idArt, $fRecibido    )
    {
        ...
        ->where(['nIdTraspaso' => $id, 'nIdArticulo' => $idArt ])
        ->update();
    }
    ...
    {
        $join1 = 'alarticulo a';
        $wjoin1 = 'a.nIdArticulo = altraspasodetalle.nIdArticulo';
        if($paginado === 0)
            return $this
                ->join($join1,$wjoin1,'left')
                ->where(['nIdTraspaso' =>  $id])
                ->findAll();
        else
            return $this
                ->join($join1,$wjoin1,'left')
