...
class Login extends BaseController
{
    ...
    public function index()
    {
        ...
        $validaSuc = $mdlConfig->getRegistros(6);

        $data[] = [];
        $bSeleccionSucursal = 0;
        if ($validaSuc !== null)
            if ($validaSuc['fValor'] === '1') {
                $bSeleccionSucursal = 1;
                $data['regSucursales'] = $mdlSucursal->getRegistros();
            }
        if (strtoupper($this->request->getMethod()) === 'POST') {
            $a = $this->validaCampos($bSeleccionSucursal);
            if ($a === false) {
                $session = session();
                //Ir a desktop
                ...
                if ($user['sPsw'] === md5($this->request->getVar('sPsw'))) {
                    if ($bSeleccionSucursal)
                        $regSuc = $mdlSucursal->getRegistros($this->request->getVar('nIdSucursal'), false, false, true);
                    else
                        $regSuc = $mdlSucursal->getRegistros($user['nIdSucursal'], false, false, true);
                    set_cookie('nIdUsuario', $user['nIdUsuario'], 0, '', '/', '', false, true);
                    set_cookie('nIdSucursal', $regSuc['nIdSucursal'] ?? '0', 0, '', '/', '', false, true);
                    set_cookie('nIdPerfil', $user['nIdPerfil']);
    ...
    public function validaCampos($bValidaSucursal)
    {
        ...
        if ($bValidaSucursal) {
            $reglas['nIdSucursal'] = 'required|greater_than_equal_to[0]';
            // $aReglaSuc = ['nIdSucursal' => 'required|greater_than_equal_to[0]'];
            $reglasMsj['nIdSucursal'] = [
                'required'   => 'Sucursal requerida',
                'greater_than_equal_to' => 'Debe seleccionar la sucursal'
            ];
            // $aMensajeSuc = ['nIdSucursal' => [
            //     'required'   => 'Sucursal requerida',
            //     'greater_than_equal_to' => 'Debe seleccionar la sucursal'
            // ]];
        }
