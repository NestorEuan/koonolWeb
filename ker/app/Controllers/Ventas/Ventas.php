...
use App\Models\Ventas\VentasCambioMdl;
...
class Ventas extends BaseController
{
        ...
        */
        ...
        foreach ($r as $a) {
            $_SESSION['Ventas']['pago']['tipos'][$a['nIdTipoPago']] = [
                $a['sTipoSAT'],
                round(floatval($a['nComision']) / 100, 4),
                $a['sLeyenda'],
                round(floatval($a['nComision']), 2)
            ];
            if ($a['sTipoSAT'] == '01') {
                if ($a['cTipo'] == '0') {
    ...
    {
        $this->initVarSesion();
        // se agrega cliente publico de entrada
        $mdlCliente = new ClienteMdl();
        $datos =  $mdlCliente->getRegistros(2);
    ...
    private function initDataVenta($recargaArticulos = false)
    {
        ...
        // se cargan las ventas en espera
        $mdlVentasEspera = new VentasEsperaMdl();
        $data['rEnEspera'] = $mdlVentasEspera->select('dtAlta, DATE_FORMAT(dtAlta, \'%Y%m%d%H%i%s\') AS sdtAlta')->where('nIdUsuario', $this->nIdUsuario)
            ->where('nIdSucursal', $this->nIdSucursal)
            ->orderBy('dtAlta', 'asc')
            ->findAll();
        $data['tabventaEnEspera'] = 'x';
        if (isset($_SESSION['Ventas']['ventaEnEspera'])) {
            $data['tabventaEnEspera'] = $_SESSION['Ventas']['ventaEnEspera']['tab'];
        }
        if ($recargaArticulos) {
            $mdlArticulo = new ArticuloMdl();
            $regArt = $mdlArticulo->select('nIdArticulo AS id, sDescripcion AS des, sCodigo AS cod')->findAll();
            array_walk($regArt, function (&$item) {
                $item['des'] = esc($item['des']);
            });
            $data['regArticulos'] = $regArt;
        }
        return $data;
    }

    public function index($soloCuerpoVentas = false)
    {
        $this->validaSesion();
        if (($l = $this->validaTurnoCorte(true))) return $l;

        if (!isset($_SESSION['Ventas'], $_SESSION['Ventas']['foc'])) {
            $this->resetVenta();
        }
        $data = $this->initDataVenta(true);
        if ($soloCuerpoVentas) {
    ...
    public function indexasp()
    {
        ...
        echo view('templates/header', $this->dataMenu);
        echo view('ventas/ventasasp', $data);
        echo view('templates/footer', $this->dataMenu);
        return;
    }
    ...
    {
        // se reinicia la comision bancaria cuando se recalcula todo
        $cp = $this->initCalculoPrecio($idProdACalcular === false);
        $asum = [
    ...
    protected function calcularPrecio($k, $r, &$asum)
    {
        ...
        if (isset($_SESSION['Ventas']['lstArtCot'][$k])) {
            ...
            $nPrecio = $p == null ? 0 : $p[$asum['campoPrecio']];
        }


        if ($noAplicarComiDesc === false) {
            $nPrecio = round(floatval($nPrecio), 2);
            $nImp = round($r[3] * $nPrecio, 2);

            $importeComisionBancaria = round($nImp * $asum['porcenComisionBancaria'], 2);
            $nImp0 = $nImp + $importeComisionBancaria;

            $descProd = round($_SESSION['Ventas']['lstArt'][$k][9] / 100, 2);
            $nDescP = round($nImp0 * $descProd, 2);
            $nImp1 = $nImp0 - $nDescP;

            $nDescG = round($nImp1 * $asum['descGral'], 2);
            //$nIva = round(($nImp1 - $nDescG) * $factorIva, 2);
            $nIva = 0;
        } else {
            $nPrecio = round(floatval($nPrecio), 3);
            $nImp = round($r[3] * $nPrecio, 3);
            $nIva = 0;
            $nDescP = 0;
            $nDescG = 0;
    ...
    protected function calcularImportes($tipoMov = 'A', $aLstPagos = false, $idProdACalcular = false)
    {
        $factorIva = $_SESSION['Ventas']['factura']['impuestoIVA']['valor'];
        // $cp = $this->initCalculoPrecio($tipoMov == 'A');
        $cp = $this->initCalculoPrecio($tipoMov == 'A');
        $tipoListaPrecios = $cp['idTipoLista'];
        $campoPrecio = $cp['campoPrecio'];
        $porcenComisionBancaria = $cp['porcenComiBanc'];
        ...
        foreach ($_SESSION['Ventas']['lstArt'] as $k => $r) {
            if (isset($_SESSION['Ventas']['lstArtCot'][$k])) {
                ...
                $nPrecio = $p == null ? 0 : $p[$campoPrecio];
            }

            $nPrecio = round(floatval($nPrecio), 2);
            $importeComisionBancaria = round($nPrecio * $porcenComisionBancaria, 2);
            $nSumComisioBancaria += $importeComisionBancaria;
            $nPrecio = $nPrecio + $importeComisionBancaria;

            $_SESSION['Ventas']['lstArt'][$k][2] = $nPrecio;
            $nImp = round($r[3] * $nPrecio, 2);

            $descProd = round($_SESSION['Ventas']['lstArt'][$k][9] / 100, 2);
            $nDescP = round($nImp * $descProd, 2);
            $nImp1 = $nImp - $nDescP;
            $nSumSub += $nImp1;

            $nDescG = round($nImp1 * $descGral, 2);
            //$nIva = round(($nImp1 - $nDescG) * $factorIva, 2);
            $nIva = 0;

            $nSumDesc += $nDescG;
            $nSumIva += $nIva;
            if ($nImp == 0) $bndPrecioEnCero = true;
            $_SESSION['Ventas']['lstArt'][$k][8] = $nIva;
            $_SESSION['Ventas']['lstArt'][$k][10] = $nDescP;
            $_SESSION['Ventas']['lstArt'][$k][11] = $nDescG;
        }
        $_SESSION['Ventas']['pago']['comision'] = $nSumComisioBancaria;
        $_SESSION['Ventas']['pago']['sub'] = $nSumSub;
        $_SESSION['Ventas']['pago']['des'] = $nSumDesc;
        $_SESSION['Ventas']['pago']['iva'] = $nSumIva;
        $_SESSION['Ventas']['pago']['tot'] = $_SESSION['Ventas']['pago']['sub']
            - $_SESSION['Ventas']['pago']['des']
            + $_SESSION['Ventas']['pago']['iva'];

        $b = $this->recalculaPagos(
            $aLstPagos === false ? $_SESSION['Ventas']['pago']['lst'] : $aLstPagos,
            $tipoMov
        );
        if ($bndPrecioEnCero) {
            $_SESSION['Ventas']['pago']['msjErr'] = 'No puede haber precios unitarios en cero.';
            ...
            $_SESSION['Ventas']['pago']['msjErr'] = $b['err'];
        }
    }
    ...
    public function agregaCliente($nIdCliente = false)
    {
        function conCotizacion($nombre)
        {
            $_SESSION['Ventas']['cli']['nomManual'] = $nombre;
        ...
        function otrosFormatos($idSucursal, $idCliente)
        {
            $model = new ClienteMdl();
            $datos =  $model->getRegistros($idCliente);
            $_SESSION['Ventas']['cli']['id'] = $datos['nIdCliente'];
            $_SESSION['Ventas']['cli']['nom'] = $datos['sNombre'];
            $_SESSION['Ventas']['cli']['email'] = $datos['email'];
            $_SESSION['Ventas']['cli']['dire'] = $datos['sDireccion'];
        ...
        $this->validaSesion();
        $retJSON = $this->request->getVar('retJSON') ?? false;
        if ($retJSON && ($_SESSION['Ventas']['docu']['modoDocu'] != 'C')) {
            $id = $this->request->getVar('id') ?? '0';
            $mdlCliente = new ClienteMdl();
            $reg = $mdlCliente->where('nIdCliente', $id)->orLike('sNombre', $id, 'none', null, true)->first();
            if ($reg == null) {
                return json_encode([
                    'ok' => '0',
                    'tipomsj' => 'MSJ',
            ...
            $id = $_POST['nIdCliente'];
            $nombre = $_POST['nIdCliente'];
        }
        if ($_SESSION['Ventas']['docu']['modoDocu'] == 'C') {
            conCotizacion($nombre);
        } else {
            otrosFormatos($this->nIdSucursal, $id);
        }

        $this->calcularPrecios();
        $this->initPagos();
        if ($retJSON) {
            $data = $this->initDataVenta();
            return json_encode(['ok' => '1', 'data' => $data]);
        }
        $this->index();
    }

    public function buscaCodigo($cod, $cant)
    {
        $this->validaSesion();
        if (($this->request->getVar('nombreventas') ?? false)) {
            $cod = trim($this->request->getVar('nombreventas'));
        }
        $retJSON = $this->request->getVar('retJSON') ?? false;
        $mdlArticulo = new ArticuloMdl();
        $reg = $mdlArticulo->where('sCodigo', $cod)->orLike('sDescripcion', $cod, 'left', null, true)->first();
        if ($reg == null) {
            if ($retJSON) {
                return json_encode([
                    'ok' => '0',
                    'tipomsj' => 'MSJ',
                    'msj' => 'Codigo no encontrado.'
                ]);
            } else {
                return 'nOoKMSJCodigo no encontrado.';
            }
        }
        if ($reg['cSinExistencia'] == '0') {
            $mdlInventario = new InventarioMdl();
            $regInv = $mdlInventario->getRegistros($reg['nIdArticulo'], $this->nIdSucursal);
            // if (count($regInv) == 0) return 'nOoKMSJNo hay en existencia en esta sucursal.';
            $valExistencias = round(floatval($regInv[0]['fExistencia'] ?? 0), 3);
            // if ($val <= 0) return 'nOoKNo hay en existencia en esta sucursal.';
            if (isset($_SESSION['Ventas']['lstArt'][$reg['nIdArticulo']]))
                $nCant2 = $_SESSION['Ventas']['lstArt'][$reg['nIdArticulo']][3] + $cant;
            else
                $nCant2 = $cant;
            if ($valExistencias < $nCant2) {
                $html = $this->valExistencia($reg['nIdArticulo'], $nCant2);
                if ($retJSON) {
                    return json_encode([
                        'ok' => '0',
                        'tipomsj' => 'ALERTA',
        ...
        $this->agregaArticulo(true, $reg['nIdArticulo'], $cant);
        if ($retJSON) {
            $data = $this->initDataVenta();
            return json_encode([
                'ok' => '1',
    ...
    public function agregaArticulo($soloCuerpoVentas = false, $idArticulo = false, $nCant = false)
    {
        $this->validaSesion();
        $retJSON = $this->request->getVar('retJSON') ?? false;
        $mdlArticulo = new ArticuloMdl();
        if ($soloCuerpoVentas === false) {
        ...
        if (isset($_SESSION['Ventas']['lstArt'][$idArticulo]))
            $_SESSION['Ventas']['lstArt'][$idArticulo][3] +=  $nCant;
        else
            $_SESSION['Ventas']['lstArt'][$idArticulo] = [
                $idArticulo,
                trim($regArt['sDescripcion']),
                0,      // precio
                $nCant,
                0,
                0,   // exist. disponible, exist. en otros almacenes, 
                0,
                0,   // Cant. a entregar, Cant a enviar
                0,
                0.0, // IVA, descuento prod.
                0,
                0,
                0, // Imp. Desc. prod., Imp. Desc. Gral., Saldo por Surtir
                '0',
                0,
                false,       // articulo en modo ENV, exis. fisica sucursal.
                $regArt['cConDescripcionAdicional'],
                round(floatval($regArt['fPeso']), 2),   // peso del producto por unidad
                0   // Imp. comision bancaria
            ];
    ...
    public function agregaPago()
    {
        ...
        if ($idTipoPago == $_SESSION['Ventas']['pago']['idCredito']) {
            if ($_SESSION['Ventas']['cli']['tipoCli'] == 'P') {
                ...
                if ($retJSON) {
                    return json_encode(['ok' => '0', 'msj' => $_SESSION['Ventas']['pago']['msjErr']]);
                }
                $this->index();
                return;
            }
    ...
    public function entrega($directa = '')
    {
        $this->validaSesion();
        ...
        $this->assignaDatosEnvio('0', $bEntrega, $directa);
    }
    ...
    public function entregaTodo()
    {
        ...
        foreach ($_SESSION['Ventas']['lstArt'] as $k => $v) {
            ...
            if ($bndValExistencia === false) {
                ...
                $_SESSION['Ventas']['lstArt'][$k][6] = round(floatval($v[4]), 3);
            }
            $_SESSION['Ventas']['lstArt'][$k][7] = 0;
            $_SESSION['Ventas']['lstArt'][$k][12] =
                $_SESSION['Ventas']['lstArt'][$k][3] -
    ...
    private function assignaDatosEnvio($bEnvio, $bEntrega, $directa = '')
    {
    ...
    public function entregaArt($id, $tipoMov, $cant)
    {
        ...
        } elseif (($bndValExistencia && ($regUsu['bDaVtaSinDisponibles'] == '0') && ($cant > $_SESSION['Ventas']['lstArt'][$id][4]))) {
            $msjErr = 'No hay suficientes productos disponibles en almacén';
        } elseif (($bndValExistencia && ($regUsu['bDaVtaSinDisponibles'] == '1') && ($cant > $_SESSION['Ventas']['lstArt'][$id][14]))) {
    ...
    public function initCalculoPrecio($bReiniciaComisionBancaria = false)
    {
        if ($_SESSION['Ventas']['docu']['modoDocu'] == 'C') {
    ...
    public function valExistencia($id, $cant)
    {
        ...
        if ($nPrecio == 0 && $rArt['cImporteManual'] == '0') {
            echo 'nOoKMSJEl articulo no tiene precio';
        }
        ...
        if ($retJSON) {
            return view('ventas/articulosDisponibles', $data);
        } else {
            echo 'nOoK';
    ...
    public function reimprimirLastDoc()
    {
        ...
        if ($retJSON) {
            return json_encode([
                'tipo' => $_SESSION['lastDoc']['tipo'],
                'id' => $_SESSION['lastDoc']['id'],
            ]);
        }
    ...
    public function consultaRemision($idVenta, $reiniciaVenta = true, $dest = 'venta', $nIdEntrega = false)
    {
        ...
        foreach ($rvd as $k => $v) {
            ...
            $regConfig = $mdlConfig->getRegistrosByName('PRECIOTAPADO', '1');
            $fFactor = 1;
            if (count($regConfig) == 1)
                $fFactor = $regConfig[0]['fValor'] == '1' ? 1.2 : 1;

            $nPrecioT = round(floatval($pt == null ? round(floatval($v['nPrecio']) * $fFactor, 2) : $pt['fPrecioTapado']), 2);
            ...
            if ($nPrecioT < $nPrecio) $nPrecioT = round($nPrecio * $fFactor, 2);
            ...
            if ($dest == 'entregaos') {
                ...
                $av = array_filter($detEntrega, function ($row) use ($idArti) {
                    return $row['nIdArticulo'] == $idArti;
                });
                if (count($av) > 0) {
                    $det[$nIndDet][3] = round(floatval($av[0]['fCantidad']), 3);
                }
            }
            if (round(floatval($v['nEntrega']), 3) > 0) {
                $bHayEntrega = true;
                if (round(floatval($v['nEntrega']), 3) < $nCant) $bSeEntregaTodo = false;
            } else {
                $entregaConCero = true;
            }
            if (round(floatval($v['nEnvio']), 3) > 0) {
                $bHayEnvio = true;
                if (round(floatval($v['nEnvio']), 3) < $nCant) $bSeEnviaTodo = false;
            } else {
                $envioConCero = true;
            }
        }
        ...
        $mdlVentaCambio = new VentasCambioMdl();
        $rvCam = $mdlVentaCambio->where('nIdVentas', $idVenta)->first();
        if ($rvCam != null) {
            $data['cambio'] = round(floatval($rvCam['nCambio']), 2);
            $data['efecReal'] = $data['impTotal'];
            $data['efecPagado'] = round(floatval($rvCam['nImporteEntregado']), 2);
        }
        return $data;
    }

    public function imprimeRemision($idVenta, $reiniciaVenta = true, $dest = 'venta', $nIdEntrega = false, $directa = '')
    {
        $this->validaSesion();
        ...
        // si es reimpresion
        if ($dest == 'venta' && !$reiniciaVenta) $_SESSION['Ventas']['pago']['cambio'] = 0.0;
        $mdlConfig = new ConfiguracionMdl();
        $regConfig = $mdlConfig->getRegistrosByName('PRECIOTAPADO', true);
        if (count($regConfig) == '1') {
            $data['dat']['conPrecioTapado'] = intval($regConfig[0]['fValor']) == 1;
        } else {
            $data['dat']['conPrecioTapado'] = true;
        }
        // echo view('templates/header', $this->dataMenu);
    ...
    public function guardaVenta($directa = '')
    {
        $session = session();
        $retJSON = $this->request->getVar('retJSON') ?? false;
        if ($this->nIdSucursal === null) {
            if ($retJSON) {
                echo json_encode([
                    'ok' => '0',
                    'msj' => 'Falta seleccionar sucursal',
                    'iniciaSesion' => '1',
                ]);
                return;
            }
        ...
        $mdlVentasEspera = new VentasEsperaMdl();
        if ($_SESSION['Ventas']['docu']['modoDocu'] !== 'C') {
            $ret = $this->guardaRemision($rSuc, $mdlSucursal, $idSuc, $directa);
            if ($mdlSucursal->db->transStatus() === true) {
                if ($ret['ok'] == '0') {
                    ...
                    if (isset($_SESSION['Ventas']['ventaEnEspera'])) {
                        $mdlVentasEspera
                            ...
                            ->where('nIdSucursal', $this->nIdSucursal)
                            ->where('dtAlta', $_SESSION['Ventas']['ventaEnEspera']['id'])
                            ->delete(null, true);
                    }
                    if ($retJSON) {
                        $this->resetVenta();
                    ...
                    $this->imprimeRemision($ret['id'], true, 'venta', false, $directa);
                    return;
                ...
                if ($retJSON) {
                    ...
                    echo json_encode([
                        'ok' => '0',
                        'msj' => $_SESSION['Ventas']['pago']['msjErr'],
                    ]);
                    return;
                }
                $this->index();
     ...
     */
    public function guardaRemision($rSuc, SucursalMdl $mdlSucursal, $idSuc, $directa = '')
    {
        $mdlVentas = new VentasMdl();
        $mdlVentasdet = new VentasDetMdl();
        ...
        $nIdDirEntrega = 0;
        $bEnvio = $this->request->getVar('hayEnvio') ?? '0';
        ...
        foreach ($_SESSION['Ventas']['lstArt'] as $k => $v) {
            ...
            $idVentasDet = $mdlVentasdet->insert([
                ...
                'nEnvio' => $directa ? 0 : $v[7],
                'nEntrega' =>  $directa ? $v[3] : $v[6],
                'nPorEntregar' =>  $directa ? 0 : round($v[3] - $v[6] - $v[7], 3),
            ...
            if ($v[15] !== false) {
                foreach ($v[15] as $vv) {
                    $mdlVentasdetRel->insert([
                        ...
                        'nEnvio' =>  $directa ? 0 : $vv[7],
                        'nEntrega' =>  $directa ? round($vv[7] + $vv[6], 2) : $vv[6]
                    ]);
                }
            }
        ...
        if ($_SESSION['Ventas']['pago']['cambio'] > 0) {
            $mdlVentaCambio = new VentasCambioMdl();
            $mdlVentaCambio->insert([
                'nIdVentas' => $idVentas,
                'nImporteEntregado' => $_SESSION['Ventas']['pago']['efec'],
                'nCambio' => $_SESSION['Ventas']['pago']['cambio'],
            ]);
        }
        if ($nSumAnticipos > 0) {
            $r = $mdlCliente->getRegistros($_SESSION['Ventas']['cli']['id']);
        ...
        $mdlInventario = new InventarioMdl();
        $nIdSucursalEntrega = $this->request->getVar('nIdSucursalEntrega') ?? $idSuc;
            ...
            foreach ($_SESSION['Ventas']['lstArt'] as $k => $v) {
                ...
                if ($v[15] === false) {
                    if (!$directa) {
                        ...
                        $ar2d2 = $inventario->getArticuloSucursal(intval($v[0]), $nIdSucursalEntrega);
                    ...
                    $retCompromete = $this->comprometeInv($v[0], '0', $nIdSucursalEntrega, $v[1], $v[4], $v[14], $v[6], $directa);
                    if ($retCompromete['ok'] == '0') return $retCompromete;
                } else {
                    foreach ($v[15] as $vv) {
                        if (!$directa)
                        ...
                        $retCompromete = $this->comprometeInv($vv[0], '0', $nIdSucursalEntrega, $vv[1], $vv[4], $vv[3], $vv[6], $directa);
                        if ($retCompromete['ok'] == '0') return $retCompromete;
                    }
                }
            }
    ...
    private function comprometeInv($idArt, $modoEnv, $idSuc, $cDes, $nDisponible, $nExistencia, $nCant, $directa = '')
    {
        $mdlInventario = new InventarioMdl();
        $mdlArticulo = new ArticuloMdl();
        ...
        if ($modoEnv == '0' && $rArt['cSinExistencia'] == '0') {  // si no tiene modo ENV  se valida inventario
            ...
            if ($directa)
                $mdlInventario->updtArticuloSucursalInventario($idSuc, intval($idArt), -1 * $nCant);
            else
                $mdlInventario
                    ->set('fExistencia', $nComprometido)
    ...
    public function cambiaPrecio($id)
    {
        ...
        if (strtoupper($this->request->getMethod()) === 'POST') {
            $_SESSION['Ventas']['lstArtCot'][$id] = [
                round(floatval($this->request->getVar('valImporte')), 2),
    ...
    public function aplicaComision($id)
    {
        $this->validaSesion();
        $retJSON = $this->request->getVar('retJSON') ?? false;
        if ($retJSON) $id = $this->request->getVar('id');
    ...
    public function buscaCotizacion($idFolioCotizacion, $modoDocu = 'R', $tipoReturn = 'json')
    {
        ...
        foreach ($rd as $k => $v) {
            ...
            $a[$v['nIdArticulo']] = [
                ...
                0,
                0,
                round(floatval($v['nPorcenDescuento']), 2),
                0,
                0,
                0, // Imp. Desc. prod., Imp. Desc. Gral., Saldo por Surtir
                '0',
    ...
    public function buscarticulodlg()
    {
        echo view('ventas/buscarticulodlg');
    }
    ...
    public function enEspera($tipoaccion, $id = '')
    {
        $mdlEspera = new VentasEsperaMdl();
        $this->validaSesion();
        $retJSON = $this->request->getVar('retJSON') ?? false;
        if ($tipoaccion == 'a') {
            $r = [
                ...
                'nIdSucursal' => $this->nIdSucursal,
                'nIdCliente' => 0,
                'data' => json_encode($_SESSION['Ventas'])
            ];

            if (isset($_SESSION['Ventas']['ventaEnEspera']['id'])) {
                $mdlEspera
                    ->where('nIdUsuario', $this->nIdUsuario)
                    ->where('nIdSucursal', $this->nIdSucursal)
                    ->where('dtAlta', $_SESSION['Ventas']['ventaEnEspera']['id'])
            ...
            $val = $this->request->getVar('tabEsperaSeleccionado');
            $val = substr($val, 3);
            $val = substr_replace($val, '-', 4, 0);
            $val = substr_replace($val, '-', 7, 0);
            $val = substr_replace($val, ' ', 10, 0);
            $val = substr_replace($val, ':', 13, 0);
            $val = substr_replace($val, ':', 16, 0);
            $reg = $mdlEspera->select('data')
                ->where('nIdUsuario', $this->nIdUsuario)
                ->where('nIdSucursal', $this->nIdSucursal)
                ->where('dtAlta', $val)
                ->findAll();
            if ($reg != null && count($reg) > 0) {
                $_SESSION['Ventas'] = json_decode($reg[0]['data'], true);
                $_SESSION['Ventas']['ventaEnEspera'] = [
                    'id' => $val,
                    'tab' => $this->request->getVar('tabEsperaSeleccionado'),
                ];
            }
            if ($retJSON) {
                $data = $this->initDataVenta();
