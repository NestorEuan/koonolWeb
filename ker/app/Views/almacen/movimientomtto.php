...
$fIVA = 0;
$pago['completar'] = 0;
$deshabilitaCampos = $modo === 'A' ? ($proveedor['nIdProveedor'] == 0 ? (in_array($operacion, array('entrada', 'salida', 'capturainv')) ? '' : 'disabled') : '') : 'disabled';
...
$aKeys = [
    ...
    'capturainv' => ['idInventarioCaptura', 'dAplicacion', 'del usuario'],
];
...
<div class="<?= $containerClass ?>">
    <?php if (isset($printerMode)) : ?>
        ...
        <?php if (!in_array($operacion, array('entrada', 'salida', 'capturainv'))) : ?>
            <div class="row px-4">
                <div class="col">
    ...
    <div class="row h-75">
        <div class="<?= $borderClass ?>">
            <?php if (!isset($printerMode)) : ?>
                ...
                <form action="<?= base_url('/movimiento/agregaArticulo/' . $operacion . '/' . strtolower($modo)) ?>" method="get" id="frm00" autocomplete="off">
                    ...
                    <div class="input-group ">
                        ...
                        <?php if ($operacion === 'capturainv') : ?>
                            <input type="hidden" name="fExistencia" id="fExistencia" value="">
                        <?php endif; ?>
                ...
                <table class="<?= $tblClass ?>" id="tbl">
                    <thead>
                        <tr>
                            ...
                            <?php if ($operacion === 'capturainv') :?>
                                <th class="text-center" scope="col">Exist.Sistema</th>
                                <th class="text-center" scope="col">Exist.Real</th>
                            <?php else: ?>
                    ...
                    <tbody>
                        ...
                        <?php else : ?>
                            <?php foreach ($registros as $r) : ?>
                                ...
                                <?php if ($operacion === 'capturainv') :?>
                                    <td class="text-end pe-5"> <?= $r['fExistencia'] ?> </td>
                                <?php endif; ?>
                                    ...
                                    <?php if (!(isset($printerMode) || $modo !== 'A')) : ?>
                                        <td>
                                            <i class="bi bi-trash-fill text-danger me-3 " data-bs-toggle="modal" data-bs-target="#mdConfirma" data-llamar="movimiento/borraArticulo/<?= $operacion . '/' . $r['nIdArticulo'] ?>" data-mod-msj="Confirma borrar el registro?" style="cursor:pointer;"></i>
                                        </td>
                                    <?php endif; ?>
                                </tr>
                            <?php endforeach; ?>
        ...
        <?php if (!isset($printerMode)) : ?>
            <div class="col" style="max-width: 350px;">
                <div class="container-fluid border rounded pt-3  bg-light">
                    ...
                    <form action='<?= base_url("movimiento/agregaProveedor/$operacion/" . strtolower($modo)) ?>' method="post" id="frmBuscaProveedor" autocomplete="off">
                        <div class="row mb-1">
                            <?php if ($bVerTodasSuc) : ?>
                                <div class="col input-group input-group-sm">
                                    <label for="nIdSucursal" class="form-label m-1">Sucursal</label>
                                    <?php generaCampoTexto('nIdSucursal', $error ?? false, 'select', $registro ?? null, $modo ?? '', 'form-control m-1', "placeholder='Sucursal' " . $deshabilitaCampos, $lstSucursales, 'sDescripcion'); ?>
                                </div>
                            <?php else : ?>
                                <span class="col-12 fw-bold text-center">
                                    <?php generaCampoTexto('nIdSucursal', $error ?? false, 'hidden', $registro ?? null, $modo, '', $deshabilitaCampos); ?>
                                    <?= $registro['sDescripcion'] ?>
                                </span>
                            <?php endif ?>
                        </div>
                        ...
                        <?php if (!in_array($operacion, array('entrada', 'salida', 'capturainv'))) : ?>
                            <div class="col">
                                <div for="dlProveedores" class="form-label"><?= $operacion === 'traspaso' ? 'Enviar a' : 'Proveedor' ?></div>
                                <div class="input-group ">
                        ...
                        <div class="col">
                            ...
                            <div class="input-group ">
                                <?php generaCampoTexto("$datefld", $error ?? false, 'date', $registro ?? null, $modo,'', $deshabilitaCampos); ?>
                            </div>
                        </div>
                        <div class="col">
                        ...
                        <?php if($operacion == 'entrada') : ?>
                            <div class="row mb-1">
                                <div class="col-7">Total Importe</div><span class="col-5 text-end fw-bold"><?= number_format(floatval($registro['fTotal']), 2) ?></span>
                            </div>
                        <?php endif; ?>
                        <hr style="height:1px; background-color:black; width:100%; margin: 2px 0 0 0;">
                    </form>
                    <?php if( isset($regcancel) && $regcancel != null) : ?>
                                <div class="col">
                                <textarea class="form-control is-valid" id="obscancel" name="obscancel" rows="2" readonly > <?= $regcancel[0]['sObservacion']; ?> </textarea>
                                </div>
                    <?php endif; ?>
                    ...
                    <div class="text-center my-1">
                        ...
                        <?php if($operacion === 'capturainv') : ?>
                            <button class="btn btn-outline-primary btn-sm me-3" data-bs-toggle="modal" data-bs-target="#mdConfirma" data-llamar="/movimiento/limpiaOperacion/<?= $operacion ?>/<?= $masterkey ?>" data-mod-msj="Desea limpiar la venta? Toda la captura se eliminará!!">
                                Generar movimientos
                            </button>
                        <?php endif; ?>
                        ...
                        <?php endif; ?>
                        <?php if( $operacion === 'entrada' && $modo === 'C') :?>
                            <button class="btn btn-sm btn-outline-success me-3" data-bs-toggle="modal" data-bs-target="#mdConfirma" data-llamar="<?= $frmURL ?>" data-mod-msj="Guardar la información!!">
                                Cancelar
                            </button>
                        <?php endif; ?>
    ...
    $(document).ready(function() {
        ...
        const selArticulo = {
            ...
            seleccionaOpcion: function(valor) {
                ...
                if (b) {
                    ...
                    if (operacion == 'entrada' || operacion == 'compra') {
            ...
            onInput: function(e) {
                ...
                if (e.target.value && e.target.value.trim().length > 1) {
                    ...
                    $.get(baseURL + '/articulo/buscaNombre/' + encodeURIComponent(e.target.value.trim()), {}, null, 'json')
            ...
            onKeydown: function(e) {
                ...
                function buscaCodArticulo(id) {
                    ...
                    $.post(baseURL + '/articulo/leeIdBarCode/' + id, {}, null, 'json').
                    done(function(data, textStatus, jqxhr) {
                        if (data.ok == '0') {
        ...
        const valCantidad = {
            ...
            onKeydown: function(e) {
                if ((e.which == 13 || e.which == 9) && e.target.value > 0) {
                    ...
                    let operacion = '<?= $operacion ?>';
                    if (operacion == 'traspaso' || operacion == 'capturainv') {
                        let sucursalida = '<?= $nIdSucursal ?>';
                        // if (operacion == 'salida')
                        ...
                        // http://proyectos/repo/ferromat/web/public/movimiento/agregaArticulo/traspaso/a
                        $.post(baseURL + '/existencia/' + sucursalida + '/' + id, {}, null, 'json').
                        done(function(data, textStatus, jqxhr) {
                            //console.log(data.registro[0]);
                            if (data.ok === '0' && operacion != 'capturainv') {
                                miGlobal.muestraAlerta('No hay artículos en existencia', 'movtomtto', 1100);
                                $('#nCant').select();
                                valCantidad.bandCantidad = false;

                            } else {
                                if (parseFloat(data.registro[0].fExistencia) < parseFloat(e.target.value) && operacion != 'capturainv') {
                                    miGlobal.muestraAlerta('Solo hay ' + data.registro[0].fExistencia + ' en existencia', 'movtomtto', 1100);
                                    valCantidad.bandCantidad = false;
                                } else {
                                    miGlobal.agregaCamposHidden('#<?= $datefld ?>, #sObservacion, #nIdSucursal', '#frm00');
                                    $('#fExistencia').val(data.registro[0].fExistencia);
                                    $('#frm00')[0].submit();
                                }
                            }
                            miGlobal.toggleBlockPantalla();
        ...
        const movTabla = {
            ...
            enviar: function(e) {
                ...
                <?php if (in_array($operacion, array('entrada', 'salida', 'capturainv')) == false) : ?>
