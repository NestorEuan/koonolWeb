<div class="container-fluid position-relative">
    ...
    <div class="row">
            ...
            <table class="table table-striped">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">Captura Actual<br>del Viaje</th>
                        <th class="text-center">Por surtir</th>
                        <th class="text-center">Para Envio</th>
                ...
                <tbody>
                    ...
                    <?php foreach ($regsDet as $k => $r) : ?>
                        <tr>
                            ...
                            <td><?= $r['nomArt'] ?></td>
                            <td class="text-center"><?= round(floatval($r['capturaActual']), 3) ?></td>
                            <td class="text-center"><?= round(floatval($r['porSurtir']), 3) ?></td>
                            <td style="width:100px;"><input type="text" class="form-control form-control-sm text-end py-0" name="txtEnviar<?= $r['nIdArticulo'] ?>" id="txtEnviar<?= $r['nIdArticulo'] ?>" value="<?= $r['capturada'] ?>" <?= round(floatval($r['porSurtir']), 3) == 0 ? 'disabled readonly' : '' ?>></td>
        ...
        <div class="col-lg-3 rounded bg-light px-2 py-3">
            ...
            <div class="d-flex justify-content-around align-items-center pt-3">
                ...
                <button type="button" class="btn btn-outline-primary w-50 mx-2" id="btnGuardarEnvio">Crear Envio</button>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        let appViajeEnvio = {
            ...
            nIdCliente: '<?= $regEnv['nIdCliente'] ?>',
            nIdDirEntrega: '<?= $regEnv['nIdDirEntrega'] ?>',
            ...
            onFocusOut: function(e) {
                ...
                $('#txtEntregado' + appViajeEnvio.arrEnvio[pos].nIdArticulo).text(
                    appViajeEnvio.redondea(porSurtir - nValor, 2).toString()
                );
            },
            procesando: false,
            actualizaEnvio: function() {
                // se valida la  sucursal
                if ($('#nIdSucursal').val() == '' || $('#nIdSucursal').val() == '0') {
                    miGlobal.muestraAlerta('Falta seleccionar la sucursal', 'viajesenvio', 1500);
                    $('#nIdSucursal').focus();
                    return false;
                }
                ...
                $.post('<?= $baseURL ?>', {
                        ...
                        'nIdDirEntrega': appViajeEnvio.nIdDirEntrega,
                        'det': arrDet
                    }, null, 'html')
