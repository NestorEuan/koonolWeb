                ...
                <?php if (in_array($docu['modoDocu'], ['R', 'F', 'C'])) : ?>
                    <form action="<?= base_url('ventas/agregaArticulo') ?>" method="post" id="frm00" autocomplete="off">
                        ...
                        <div class="input-group ">
                            ...
                            <input type="text" class="form-control w-75" id="dlArticulos0" name="dlArticulos0" list="dlArticulos" placeholder="Ctrl + F1 para cambiar modo, .  + o - para aumentar / disminuir cantidad." aria-label="id/codigo/nombre del producto" tabindex="3" <?= $deshabilitaCampos ?> />
                            <datalist id="dlArticulos"></datalist>
                            <i class="bi bi-upc text-dark ms-2 fs-3" title="Modo Codigo" id="modoBusqueda" ></i>
    ...
    $(document).ready(function() {
        ...
        let autoArt2 = {
            ...
            onKeydownArt: function(e) {
                ...
                function esCambioModo(e) {
                    // e.which 112 es F1
                    if(e.which == 112 && e.ctrlKey) {
                        e.preventDefault();
                        autoArt2.modo = autoArt2.modo == 'c' ? 'n' : 'c';
                        $('#dlArticulos').html('');
                        $('#dlArticulos0').val('');
                        console.log(autoArt2.modo);
                        if (autoArt2.modo === 'c') {
                            $('#modoBusqueda').removeClass('bi-alphabet').addClass('bi-upc');
                            $('#modoBusqueda').attr('title', 'Modo Codigo');
                        } else {
                            $('#modoBusqueda').removeClass('bi-upc').addClass('bi-alphabet');
                            $('#modoBusqueda').attr('title', 'Modo Descripción');
                        }
                        return true;
                    }
                    return false;
