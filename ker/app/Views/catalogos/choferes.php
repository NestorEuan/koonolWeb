    ...
    <div class="row border rounded mb-3 py-2">
        ...
        <div class="col-sm-5 mt-3 mt-sm-0 ">
            <button class="btn btn-primary bg-gradient me-3" data-bs-toggle="modal" 
                data-bs-target="#frmModal" id="btnAgregar" data-llamar="chofer/a">Agregar</button>
                    ...
                    <?php else : ?>
                        <?php foreach ($registros as $r) : ?>
                            <tr>
                                ...
                                <td><?= $r['sCelular'] ?></td>
                                <td><?= $r['sLicenciaNo'] ?></td>
                                <td><?= $r['dVencimientoLic'] ?></td>
                                <td>
                                    <i class="bi bi-pencil-fill text-primary me-3" data-bs-toggle="modal"
                                        data-bs-target="#frmModal" data-llamar="chofer/e/<?= $r['nIdChofer'] ?>" style="cursor:pointer;"></i>
                                    <i class="bi bi-trash-fill text-danger me-3 " data-bs-toggle="modal"
                                        data-bs-target="#frmModal" data-llamar="chofer/b/<?= $r['nIdChofer'] ?>" style="cursor:pointer;"></i>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
...
<script type="text/javascript">
    const appChofer = {
        ...
        init: function () {
            document.getElementById('frmModal').addEventListener('show.bs.modal', appChofer.agregar);
        },
