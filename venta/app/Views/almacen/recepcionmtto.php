                ...
                <table class="table table-striped table-hover table-sm" id="tbl">
                    <thead>
                        <tr>
                            ...
                            <?php if (($operacion === 'compra' || $operacion === 'entrada') && $modo === 'R') : ?>
                                    ...
                                    <td class="text-end">
                                        <?php if ($operacion === 'entrega') : ?>
                                            ...
                                            if ($r['cSinExistencia'] == '0') {
                                                $fDisponible = $r['fExistencia'];
                                                if ($r['fExistencia'] < $r['fPorRecibir']) {
                                                    $inventarioNodisponible = true;
                                                    $noDisp = true;
                                                }
                                            } else
                                                $fDisponible = $r['fPorRecibir'];

                                            $fMinXRecibir = $fDisponible < ($r['fPorRecibir']) ? $fDisponible : ($r['fPorRecibir']);
                                            ?>
                                            <input type="text" class="form-control input-sm py-0 <?= $noDisp ? 'bg-danger' : '' ?>" id="art-<?= $nCont ?>" data-cantidad="<?= $fMinXRecibir ?>" data-idx=<?= $nCont ?> data-fuente="cantidad" name="<?= $operacion ?>[<?= $r['nIdArticulo'] ?>]" value="<?= $fMinXRecibir ?>" <?= $fMinXRecibir > 0 ? ($operacion === 'entrega' ? 'readonly' : '') : 'readonly' ?>>
                                        <?php else : ?>
                                            <input type="text" class="form-control input-sm py-0" id="art-<?= $nCont ?>" data-cantidad="<?= $r['fCantidad'] - $r['fRecibido'] ?>" data-idx=<?= $nCont ?> data-fuente="cantidad" name="<?= $operacion ?>[<?= $r['nIdArticulo'] ?>]" value="<?= $r['fPorRecibir'] ?>" <?= $r['fPorRecibir'] > 0 ? ($operacion === 'entrega' ? 'disabled' : '') : 'disabled' ?>>
                                        <?php endif; ?>
                                    </td>
                                    <?php if (($operacion === 'compra' || $operacion === 'entrada') && $modo === 'R') : ?>
                                        <td class="text-end">
        ...
        <div class="col" style="max-width: 350px;">
            <div class="container-fluid border rounded pt-3  bg-light">
                ...
                <form action='<?= base_url("$frmURL") ?>' method="post" id="frmRecepcion" autocomplete="off">
                    ...
                    <?php if (!in_array($operacion, array('entrada', 'salida'))) : ?>
                        ...
                        <div class="col">
                            ...
                            <div class="input-group ">
                                <input class="form-control w-75" list="dlProveedores" placeholder="Escriba nombre de la sucursal" aria-label="codigo/nombre de <?= $campoproveedor ?>" id="dlProveedores0" name="dlProveedores0" value="<?= $dlProveedor ?>" <?= $operacion === 'traspaso' || $operacion === 'entrega' ? 'readonly' : '' ?> />
                    ...
                    <?php if ($operacion == 'salida') : ?>
                        <div class="col">
                            ...
                            <div class="input-group ">
                                <?php generaCampoTexto("cTipoSalida", $error ?? false, 'select', $registro ?? null, $modo, '', 'disabled', $lstTipoSalida); ?>
                            </div>
                        </div>
                    <?php endif; ?>
                    <div class="col">
                        ...
                        <div class="input-group ">
                            <?php generaCampoTexto("$datefld", $error ?? false, 'date', $registro ?? null, $modo, '', ($operacion == 'entrega' ? 'readonly' : '')); ?>
                        </div>
                    </div>
                    <div class="col">
                ...
                <div class="text-center my-1">
                    ...
                    <?php if ($operacion === 'entrega') : ?>
                        <?php if ($inventarioNodisponible) : ?>
                            <div class="alert alert-danger" role="alert">
                                No hay suficiente inventario disponible para realizar la entrega.
                            </div>
                        <?php else : ?>
                            <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#mdConfirma" data-llamar="<?= $frmURL ?>" data-mod-msj="Guardar la información!!" <?= $nRecibidos > 0 ? '' : 'disabled' ?>>
                                <?= $operacion === 'entrega' ? 'Entregar' : 'Aceptar' ?>
                            </button>
                        <?php endif; ?>
                    <?php else : ?>
                        <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal" data-bs-target="#mdConfirma" data-llamar="<?= $frmURL ?>" data-mod-msj="Guardar la información!!" <?= $nRecibidos > 0 ? '' : 'disabled' ?>>
            ...
            tblKeyDown: function(e) {
                function siguiente(e) {
                    <?php if ($modo === 'N') : ?>
                        ...
                        $('#fTotal').val(sumax);
                    <?php endif; ?>
                    let t = $(e.target); //   movimiento/recepcion/entrega/8528
