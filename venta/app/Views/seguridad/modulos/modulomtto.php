...
<hr>
<form action="" method="post" id="addModulofrm">
    <input type="hidden" id="nIdMenu" name="nIdMenu" value="<?= $id ?>">
    <div class="mb-3">
        <label for="sDescripcion" class="form-label">Descripcion</label>
        <?php generaCampoTexto('sDescripcion', $error ?? false, null, $registro ?? null, $modo); ?>
    </div>
    <div class="mb-3">
        <label for="sAbrev" class="form-label">Abreviado</label>
        <?php generaCampoTexto('sAbrev', $error ?? false, null, $registro ?? null, $modo); ?>
    </div>
    <div class="mb-3">
        <label for="sLink" class="form-label">Link</label>
        <?php generaCampoTexto('sLink', $error ?? false, null, $registro ?? null, $modo); ?>
    </div>
</form>
<hr>
...
<script type="text/javascript">
    $(document).ready(function() {
        let addModulo = {
            init: function() {
                $('#btnGuardar').on('click', addModulo.enviar);
            },
            enviar: function(e) {
                $.ajax({
                    url: '<?= $frmURL ?>',
                    method: 'POST',
                    data: $('#addModulofrm').serialize(),
                    dataType: 'html'
