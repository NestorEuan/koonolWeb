...
class PrecioArticuloMdl extends Model
{
    ...
    {
        return $this->select('alprecioarticulo.fPrecio, ' .
            'alprecioarticulo.fPrecioFactura, ' .
            'alprecioarticulo.fPrecioTapado, b.cPrecioTapadoFactura')
            ->join('alarticulo b', 'alprecioarticulo.nIdArticulo = b.nIdArticulo', 'inner')
            ->where([
                'alprecioarticulo.nIdSucursal' => intval($idSuc),
                'alprecioarticulo.nIdTipoLista' => intval($idTipoLista),
                'alprecioarticulo.nIdArticulo' => intval($idArt),
                'alprecioarticulo.fMaximo >=' => round(floatval($cant), 3)
            ])->orderBy('alprecioarticulo.fMaximo', 'ASC')
            ->limit(1)->first();
    }

    public function getlstPrecios($idSuc = false, $paginado = false, $filtroDes = false, $filtroLista = false, $soloConPrecios = false)
    {
        $sql = $this->db->query(
            'SELECT GROUP_CONCAT(' .
