...
class VentasMdl extends Model
{
    ...
    protected $allowedFields = [
        'nIdSucursal', 'nFolioRemision', 'nIdCliente', 'cEdo',
        'nSubTotal', 'nImpuestos', 'nTotal', 'bEnvio', 'nPorcenDescuento',
        'nImpDescuento', 'nPorcenComision', 'nImpComision', 'cEdoEnvio',
        'cEdoEntrega', 'nIdDirEntrega', 'cClasificacionVenta', 'cNomEntrega',
        'dtCancela', 'nIdCancela', 'sMotivoCancela', 'bVariasFacturas', 'nIdUsuario'
    ];
    ...
    public function getRegistros($id = false, DateTime $dFecIni = null, DateTime $dFecFin = null, $orden = 'DESC', $paginado = true, $idSuc = 0, $tipoFiltro = false, $filtro = false)
    {
        $this->select(
            'vtventas.*, b.sNombre, IFNULL(c.nFolioFactura, 0) as nFolioFactura, ' .
                ...
                'IFNULL(c.nIdFacturas, 0) as nIdFacturas, ' .
                ...
                'WHEN \'7\' THEN \'Cerrado\' ' .
                'END as cEstatus, ' .
                '(SELECT COUNT(al.nIdOrigen) FROM alentrega al WHERE al.nIdOrigen = vtventas.nIdVentas AND al.dtBaja IS NULL ) AS cEdoEntrega, ' .
                '(SELECT COUNT(al.nIdOrigen) FROM enenvio al WHERE al.nIdOrigen = vtventas.nIdVentas AND al.cOrigen = \'ventas\' AND al.dtBaja IS NULL ) AS cEdoEnvio, ' .
                '(SELECT SUM(al.nImporte) FROM vtventasnc al WHERE al.nIdVentas = vtventas.nIdVentas ) AS nImporteNC, ' .
        ...
        if ($id === false) {
            $a = [];
            if (($tipoFiltro !== false) && ($tipoFiltro != '4')) {
                if ($dFecIni === null) {
            ...
            $this->where($a)->orderBy('vtventas.nIdVentas', $orden);
            // if ($idUsu) {
        ...
        $this
            ->select(
                'vtventas.nIdVentas, vtventas.nIdSucursal, vtventas.nIdCliente, ' .
                    'vs.nSaldo AS fSaldo, vtventas.dtAlta, vtventas.dtAlta as dcompra, ' .
                    'cEdoEntrega, a.sDescripcion, c.sNombre, vtventas.nFolioRemision, vtventas.nTotal AS nTotalVenta, ' .
                    'a.sDescripcion AS nomSucursal, vtventas.cEdo AS cEdoVenta ' .
                    $selectAdd
            )
    ...
    {
        $a = [
            'vtventas.dtAlta >= ' => $dFecIni->format('Y-m-d'),
            'vtventas.dtAlta < ' => $dFecFin->add((new DateInterval('P1D')))->format('Y-m-d'),
            'vtventas.cEdo <>' => '5'
        ];
        if ($idSuc) {
            $a['vtventas.nIdSucursal'] = $idSuc;
    ...
    public function getRepoVentasCorte(DateTime $dFecIni, DateTime $dFecFin, $idSuc = false, $idCli = false)
    {
        ...
        $q = 'SELECT a.nIdVentas, IFNULL(b.nFolioFactura, 0) AS nFactura, b.sSerie, ' .
            ...
            ') AS cadPagos, SUM(IFNULL(vdx.nImpDescuentoProd + vdx.nImpDescuentoGral, 0)) AS sumaDescuento, a.cEdo, ' .
    ...
    public function repVentasProductoPorRemision($idProd, DateTime $dFecIni = null, DateTime $dFecFin = null, $idSuc = false)
    {
        $a = [
            'vtventas.dtAlta >= ' => $dFecIni->format('Y-m-d') . ' 00:00:00',
            'vtventas.dtAlta <= ' => $dFecFin->format('Y-m-d') . ' 23:59:59'
        ...
        $a['vtventas.cEdo <>'] = '5';
        $a['b.nIdArticulo'] = $idProd;
        $this->where($a);
        // si la factura esta cancelada no entra el detalle,
        // si la factura ya fue surtida completa pero no tiene
        // 
        $this->select('vtventas.nFolioRemision, vtventas.dtAlta, c.sNombre, b.nCant, b.nPorEntregar, b.nEntregado')
            ->join('vtventasdet b', 'vtventas.nIdVentas = b.nIdVentas', 'inner')
            ->join('vtcliente c', 'vtventas.nIdCliente = c.nIdCliente', 'inner');

        return $this->orderBy('vtventas.nIdVentas', 'ASC')->findAll();
    }

    public function repFacturasEspeciales(DateTime $fi, DateTime $ff)
    {
        $a = [
            'vtventas.dtAlta >= ' => $fi->format('Y-m-d') . ' 00:00:00',
            'vtventas.dtAlta <= ' => $ff->format('Y-m-d') . ' 23:59:59'
        ];
        $this->select(
            'vtventas.nIdVentas, vtventas.nFolioRemision, vtventas.nIdCliente, vtventas.nTotal,' .
                'f.sSerie, f.nFolioFactura, c.sNombre, DATE_FORMAT(vtventas.dtAlta, \'%Y-%m-%d\') AS fecha,' .
                'vd.nIdArticulo, IFNULL(vdd.sDescripcion, a.sDescripcion) AS sDescripcionArt,' .
                'vd.nCant, vd.nPrecio, ' .
                'IFNULL(vdx.nImpComisionTotal - vdx.nImpDescuentoProd - vdx.nImpDescuentoGral, 0) AS nDescuentoTotal',
            false
        )
            ->join('vtfacturaespecial fe', 'vtventas.nIdVentas = fe.nIdVentas', 'inner')
            ->join('vtfacturas f', 'vtventas.nIdVentas = f.nIdVentas', 'inner')
            ->join('vtcliente c', 'vtventas.nIdCliente = c.nIdCliente', 'inner')
            ->join('vtventasdet vd', 'vtventas.nIdVentas = vd.nIdVentas', 'inner')
            ->join('vtventasdetaux vdx', 'vd.nIdVentasDet = vdx.nIdVentasDet', 'left')
            ->join('vtventasdetdescripcion vdd', 'vd.nIdVentasDet = vdd.nIdVentasDet', 'left')
            ->join('alarticulo a', 'vd.nIdArticulo = a.nIdArticulo', 'inner')
            ->where($a)->orderBy('vtventas.nIdVentas', 'asc');
        return $this->findAll();
    }
