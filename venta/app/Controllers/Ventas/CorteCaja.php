...
use App\Models\Ventas\CorteCajaArqueoMdl;
...
class CorteCaja extends BaseController
{
    ...
    public function accion($tipoaccion, $id = 0)
    {
        ...
        $mdlArqueo = new CorteCajaArqueoMdl();
                ...
                if ($tipoaccion == 'a') {
                    ...
                    if (array_search($this->request->getVar('nNumCaja'), $regCajas) === false) {
                        $id = $model->insert([
                            ...
                            'nSaldoIni' => round(floatval(str_replace(',', '', $this->request->getVar('nSaldoIni'))), 2)
                        ]);
                        $this->actualizaArqueoApertura($id, $model);
                        echo 'oK';
                        return;
                    } else {
                        ...
                        $data['ultimoArqueo'] = $mdlArqueo->getUltimoArqueo($this->nIdUsuario, $this->nIdSucursal, 'C');
                        $data['idUltimoArqueo'] = $data['ultimoArqueo'][0]['nIdCorte'] ?? '0';
                        $data['ultimaCap'] = $this->initCapturaArqueo();
                    }
                } else {
                    // $tipoaccion == 'e'
                    $this->actualizaArqueoCierre($id, $model);
                    $model->update($id, [
                        'nSaldoFin' => round(floatval(str_replace(',', '', $this->request->getVar('nSaldoFin'))), 2),
            ...
            if ($tipoaccion == 'a') {
                ...
                if ($r !== null) {
                    if ($r['dtCierre'] == null) {
                        ...
                        $regCajas = $model->getCajasActivas($this->nIdSucursal);
                        $data['cajasA'] = $regCajas;
                        // se busca si tiene capturado un arqueo anterior
                        $data['ultimoArqueo'] = $mdlArqueo->getUltimoArqueo($this->nIdUsuario, $this->nIdSucursal, 'C');
                        $data['idUltimoArqueo'] = $data['ultimoArqueo'][0]['nIdCorte'] ?? '0';
                    }
                } else {
    ...
    private function actualizaArqueoCierre($id, CorteCajaMdl &$modelo)
    {

        $modelo->builder()->set('sObservaciones', 'CONCAT(vtcorte.sObservaciones, \'' .
            ($this->request->getVar('sObservaciones') ?? '') . '\') ', false)
            ->where('nIdCorte', $id)->update();

        $arr = $this->request->getVar('dat') ?? [];
        // se guarda el valor de los billetes y monedas
        $mdlArqueo = new CorteCajaArqueoMdl();
        foreach ($arr as $k => $r) {
            $aM = [
                'nIdCorte' => $id,
                'cTipoMov' => 'C',
                'cTipoValor' => substr($r[0], 0, 1),
                'nValor' => round(floatval($r[1]), 2),
                'nCant' => intval($r[2])
            ];
            $mdlArqueo->insert($aM);
        }
    ...
    private function actualizaArqueoApertura($id, CorteCajaMdl &$modelo)
    {
        $mdlArqueo = new CorteCajaArqueoMdl();
        $mdlArqueo->where([
            'nIdCorte' => ($this->request->getVar('idUltimoArqueo') ?? '0'),
            'cTipoMov' => 'C'
        ])->delete();
        if ($this->request->getVar('esDiferente') == '0') return;
        ...
        foreach ($arr as $k => $r) {
            ...
            $aM = [
                ...
                'nValor' => round(floatval($r[1]), 2),
                'nCant' => intval($r[3]),
                'nCantNueva' => intval($r[2]),
            ];
            $mdlArqueo->insert($aM);
        }
    }

    private function initCapturaArqueo()
    {
        $arr = $this->request->getVar('dat') ?? [];
    ...
    public function imprime($id)
    {
        ...
        $data['verConcentrado'] = ($this->nIdUsuario == '1' || isset($this->aPermiso['oVerTodasSuc']));
    ...
    private function corteResumenGral($id, CorteCajaMdl &$mdlCorte)
    {
        ...
        while ($bndContinua) {
            ...
            if ($nFila < $nLim2) {
                ...
                $c2 = [$r2[$nFila]['nFolioRemision'], $r2[$nFila]['nFolioFactura'], number_format(floatval($r2[$nFila]['nTotal']) - floatval($r2[$nFila]['notaCred']), 2)];
                $nSum2 += round(floatval($r2[$nFila]['nTotal']) - floatval($r2[$nFila]['notaCred']), 2);
                $sumNotasCredito += round(floatval($r2[$nFila]['notaCred']), 2);
                $bndContinua = true;
            } else {
                $c2 = ['', '', ''];
            }
            if ($nFila < $nLim3) {
                $c3 = [$r3[$nFila]['nFolioRemision'], $r3[$nFila]['nFolioRemision'], number_format(floatval($r3[$nFila]['nTotal']) - floatval($r3[$nFila]['notaCred']), 2)];
                $nSum3 += round(floatval($r3[$nFila]['nTotal']) - floatval($r3[$nFila]['notaCred']), 2);
                $sumNotasCredito += round(floatval($r3[$nFila]['notaCred']), 2);
                $bndContinua = true;
            } else {
                $c3 = ['', '', ''];
            }
        ...
        $data['lstES'] = $r;
        $r = $mdlCorte->getLstCorteEScaja($id, false);
        $data['lstESnoEfec'] = $r;

        $r = $mdlCorte->getLstCortePagos($id);
        $data['lstPag'] = $r;
        $data['notasCredito'] = $sumNotasCredito;

        $r = $mdlCorte->getLstCorteDepositos($id);
        $data['lstDep'] = $r;
        $r = $mdlCorte->getLstCorteDepositos($id, false);
        $data['lstDepNoEfec'] = $r;
        return $data;
    }
