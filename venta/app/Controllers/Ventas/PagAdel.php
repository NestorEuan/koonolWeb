...
class PagAdel extends BaseController
{
    public function index()
    {
        ...
        $idCliente = $this->request->getVar('idCliente') ?? false;
        if($idCliente == '') $idCliente = false;
        $data = [
            'registros' => $model->getRegistros(false, true, $this->nIdSucursal, false, $idCliente),
            'pager' => $model->pager,
            'puedeCancelar' => $this->aPermiso['oCancelaDepCliente'] ?? false,
            'frmURL' => base_url(),
        ];
    ...
    public function accion($tipoaccion, $id = 0)
    {
        ...
        if ($this->request->getMethod() == 'post') {
            if ($tipoaccion === 'b') {
                // Eliminar el pago del depósito 
                // solo si no ha tenido movto de pago de ese deposito.
    ...
    public function reimprimir($id)
    {
        ...
        $r = $mdlPagAdel->getRegistros($id, false, false, true);
        $this->preparaImpresion($r);
        $data = $_SESSION['impresionRECCAJA'];
        $data['esMobil'] = $this->request->getUserAgent()->isMobile();
        $data['cierraVentana'] = $data['esMobil'] == false;
        echo view('templates/header', $this->dataMenu);
